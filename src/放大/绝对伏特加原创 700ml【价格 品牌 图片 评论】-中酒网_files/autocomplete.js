...
var searchBaseUrl = "http://s.zhongjiu.cn/search/index"
...
function search(obj) {
    ...
    if (url != undefined && url != '') {
        ...
        var ad = me.data('ad');
        if (ad != null && ad.KeywordId > 0 && ad.Title == val) {
            var param = { keyword: ad.Keyword, keywordId: ad.KeywordId };
            url = baseUrl + $.param(param);
            window.location = url;
        } else {
            url = baseUrl + $.param({ keyword: val });
            window.location = url;
        }
    }
}

function fialsearch(obj) {
    var val = $('#failkeyword').val();
    if (val == "") {
        obj.focus();
        return;
    }
    var baseUrl = searchBaseUrl + "?";
    var url = baseUrl + $.param({ keyword: val });
    window.location = url;
...
function delHistory(obj) {
    ...
    $.getJSON('http://s.zhongjiu.cn/search/Suggest.ashx?action=del&callback=?&name=' + name, function (r) {
...
$(function () {
    ...
    $(".z_head_s .s_node").hover(function () {
        $(this).addClass('s_hov');
    ...
    $('#divHead2').hover(function () {
        $('#headlist2').show();
    }, function () {
        $('#headlist2').hide();
    });
    $('#divHead').hover(function () {
        $('#headList').show();
    }, function () {
        $('#headList').hide();
    });
    ...
    $('#w180').hover(function () {
        var self = $(this);
        var status = self.data("status") || false;
        if (status) {
            $('#J_mainCata').show();
        } else {
            var result = $('#category').html();
            self.append(result);
            $('#J_mainCata').show();
            var $mainCata = $("#J_mainCata"), $subCata = $("#J_subCata"), $arrowBtn = $mainCata
                .nextAll('div.J_arrowBtn'), animater = null;
            $("#J_mainCata>ul>li").bind("mouseenter", "li", function () {
                var $this = $(this), _index = $this.index(), _top = $this
                    .position().top + 40;
                $subCata.stop().css({
                    'opacity': 1
                });
                ...
                animater = setTimeout(function () {
                    var _top2 = _top, $subView = $subCata.find(".right0426_all_show").hide()
                        .eq(_index).show();
            ...
            $mainCata.parent(".w180").bind("mouseleave", function () {
                if (animater !== null) {
                    clearTimeout(animater);
                }
                animater = setTimeout(function () {
                    ...
                    $subCata.animate({
                        ...
                        $subCata.hide().find(".right0426_all_show").hide();
                        $("#J_mainCata>ul").find("li").removeClass("item item_hover").addClass("item");
                    });
                }, 200);
    ...
    var keywords = new Bloodhound({
        ...
        remote: {
            url: 'http://s.zhongjiu.cn/search/Suggest.ashx?keyword=%QUERY&callback=?',
            ajax: { dataType: 'jsonp' },
            filter: function (result) {
                ...
                for (var i = 0; i < len; i++) {
                    ...
                    o.Flag = 0;
                    o.ResetUrl = temp.ResetUrl || '';
                    o.Extra = {};
                    data.push(o);
                    var category = temp.Category;
                    var categoryLen = category.length;
                    var status = false;
                    if (o.Id != 0 && category.length > 0 && status == false) {
                        for (var j = 0; j < categoryLen; j++) {
                            var categoryTemp = category[j];
                            var c = {};
                            c.Id = o.Id;
                            c.Name = o.Name;
                            c.Amount = categoryTemp.Amount;
                            c.ResetUrl = '';
                            c.Flag = 1; //关联类目
                            c.Extra = { Id: categoryTemp.Id, Name: categoryTemp.Name };
                            data.push(c);
                            status = true;
                        }
                    }
    ...
    $('#keyword').typeahead({ autoselect: false, minLength: 0, hint: false }, {
        displayKey: 'Name',
        source: keywords.ttAdapter(),
        templates: {
            suggestion: function (obj) {
                var html = "<div><div class='s_pro'>" + obj.Name + "</div><div class='s_num'>约" + obj.Amount + "个商品</div></div>";
                if (obj.Flag == 1) {
                    html = "<div class='underline'><div class='s_pro'>在<strong>" + obj.Extra.Name + "</strong>分类中搜索</div><div class='s_num'>" + obj.Amount + "个商品</div></div>";
                }
                if (obj.Flag == 2) {
                    html = "<div class='history' onmouseover='$(this).children().children(\"a\").text(\"删除\");' onmouseout='$(this).children().children(\"a\").text(\"搜索历史\");' ><div class='s_pro'>" + obj.Name + "</div><div class='s_num'><a  data-name='" + obj.Name + "' href='javascript:void(0);' onclick='delHistory(this);'>搜索历史</a></div></div>";
                }
                return html;
            }
        }
    ...
    /*
    $("#keyword").on("typeahead:opened", function () {
    $('.tt-suggestion').hover(function () {
    $(this).find('a').html('删除');
    }, function () {
    $(this).find('a').html('搜索历史');
    });
    });
   
    $("#keyword").on("typeahead:autocompleted", function (e, o, n) {
    //console.log(e);
    });
    */
    $('#keyword').on("typeahead:cursorchanged", function (e, o, n) {
        var me = $(this);
        $('.tt-suggestion').each(function (i, n) {
            if ($(n).hasClass('tt-cursor')) {
                $(n).find('a').html('删除');
            } else {
                $(n).find('a').html('搜索历史');
            }
        ...
        var btn = $("#btn-search");
        if (o.ResetUrl != '') {
            btn.data('url', o.ResetUrl);
        } else {

            var baseUrl = searchBaseUrl + "?";

            var param = { keyword: o.Name, keywordId: o.Id };
            if (data.prevkeyword != '') {
                param.prevKeyword = data.prevkeyword;
            }
            if (o.Flag == 1) {
                var len = o.Extra.Name.split('>').length;
                if (len == 2) {
                    param.tid = o.Extra.Id;
                }
            }
            var url = baseUrl + $.param(param);
            btn.data('url', url);
            btn.data('ad', null);
        }

    });
    $('#keyword').on("typeahead:selected", function (e, o, n) {
        var me = $(this);
        var data = me.data();
        if (o.ResetUrl != '') {
            window.location = o.ResetUrl;
        } else {
            var baseUrl = searchBaseUrl + "?";
            var param = { keyword: o.Name, keywordId: o.Id };
            if (data.prevkeyword != '') {
    ...
    if (parseUri(window.location).path.toLowerCase() != "/search/index") {
        var keyword = $("#keyword");
        if (keyword) {

            keyword.typeahead('val', 'history');
            keyword.typeahead('val', '');
            keyword.typeahead('close');
        }
    } else {
        var keyword = $("#keyword");
        if (keyword) {
            //var url = window.location;
            //var param = parseUri(url);
            var searchkeyword = $('#search-keyword');
            if (searchkeyword) {
                var x = searchkeyword.text();
                keyword.val(x);
                keyword.attr("data-prevKeyword", x);
            }
        }
    }
